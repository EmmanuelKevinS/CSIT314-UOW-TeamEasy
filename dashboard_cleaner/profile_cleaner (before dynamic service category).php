<?php
session_start();
require_once "../db.php";
require_once "../classes/cleaner/CleanerProfileManager.php";
require_once "../classes/cleaner/Cleaner.php";

if (!isset($_SESSION["user_id"]) || $_SESSION["role"] !== "cleaner") {
    header("Location: ../auth.php");
    exit;
}

$userId = $_SESSION["user_id"];
$manager = new CleanerProfileManager($conn);

if ($manager->hasProfile($userId)) {
    header("Location: cleaner.php");
    exit;
}

$error = "";
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    $data = [
        "name" => $_POST["name"],
        "phone" => $_POST["phone"],
        "service_type" => isset($_POST["service_type"]) ? implode(",", $_POST["service_type"]) : "",
        "working_hours" => isset($_POST["working_hours"]) ? implode(",", $_POST["working_hours"]) : "",
        "working_days" => isset($_POST["working_days"]) ? implode(",", $_POST["working_days"]) : ""
    ];

    if ($manager->createProfile($userId, $data)) {
        header("Location: cleaner.php");
        exit;
    } else {
        $error = "Error creating profile.";
    }
}
?>

<!DOCTYPE html>
<html>
<head>
    <title>Complete Profile - Cleaner</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            background-color: #f5f6fa;
            padding: 40px;
        }
        .container {
            max-width: 750px;
            margin: auto;
            background: white;
            padding: 30px;
            border-radius: 10px;
            box-shadow: 0 0 12px rgba(0, 0, 0, 0.1);
        }

        


        .checkbox-sections {
            display: flex;
            justify-content: space-between;
            gap: 10px;
            margin-top: 20px;
            max-width: 100%;
        }

        .checkbox-sections .section {
            background: #ffffff;
            border-radius: 10px;
            padding: 15px;
            box-shadow: 0 2px 6px rgba(0,0,0,0.08);
            flex: 1;
            max-width: 200px;
        }

        .checkbox-sections .section label {
            display: block;
            margin: 4px 0;
        }

        
        form label {
            display: block;
            margin-top: 10px;
            font-weight: bold;
        }
        input[type="text"] {
            width: 100%;
            padding: 8px;
            margin-top: 5px;
            border-radius: 5px;
            border: 1px solid #ccc;
        }
        input[type="submit"], a.button {
            display: inline-block;
            margin-top: 20px;
            padding: 10px 20px;
            background-color: #007bff;
            color: white;
            text-decoration: none;
            border: none;
            border-radius: 5px;
            cursor: pointer;
        }
        input[type="submit"]:hover, a.button:hover {
            background-color: #0056b3;
        }
        .message {
            color: green;
            margin: 10px 0;
            font-weight: bold;
        }
       

        .back-link {
            text-align: center;
            margin-top: 25px;
        }
        .back-link a {
            color: #007bff;
            text-decoration: none;
        }
        .back-link a:hover {
            text-decoration: underline;
        }


        
    </style>
</head>
<body>
<div class="container">
    <h2>Complete Your Profile</h2>

    <?php if ($error): ?><div class="error"><?= htmlspecialchars($error) ?></div><?php endif; ?>
    <form method="POST">
        <label>Name:</label>
        <input type="text" name="name" required>

        <label>Phone:</label>
        <input type="text" name="phone" required>

        <div class="checkbox-sections">
            
        <div class="section">



        <label>Service Types:</label> <br>
        <?php
        $services = ["Carpet", "Bathroom", "Kitchen", "Living Room"];
        foreach ($services as $service) {
            echo "<label><input type='checkbox' name='service_type[]' value='$service'> $service</label><br>";
        }
        ?>
        </div>
        
        <div class="section">
        <label>Working Hours:</label> <br>
        <?php
        $hours = ["8AM-11AM", "11AM-2PM", "2PM-5PM", "5PM-8PM"];
        foreach ($hours as $hour) {
            echo "<label><input type='checkbox' name='working_hours[]' value='$hour'> $hour</label><br>";
        }
        ?>
        </div>

        <div class="section">
        <label>Working Days:</label> <br>
        <?php
        $days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"];
        foreach ($days as $day) {
            echo "<label><input type='checkbox' name='working_days[]' value='$day'> $day</label><br>";
        }
        ?>
        </div>
        </div>


        <input type="submit" value="Save Profile">
    </form>
      
</div>
</body>
</html>